    ...
    <script type="text/javascript">
        ...
        $(document).ready(function(){
            theGrid = $('#thegrid').DataTable({
                ...
                "autoWidth": false,
                "order": "",
                "ajax": { "url" : "{{url('patients_ipd/patientRegsiter/grid')}}", 'type':'POST', data: {_method: 'POST', _token :$("#hdnCsrfToken").data('token')}},
                "columnDefs": [
                    {
                        "targets": 0,
                        "visible": false,
                    ...
                    },
                    {
                        "render": function ( data, type, row ) {
                            return '<a href="{{ url('/patients_ipd/patientBill/') }}/'+row[0]+'/edit"><i class="fa fa-pencil-square-o" aria-hidden="true"></i>Add/Edit</a>';
                        },
                        "sortable": false,
                        "targets": 7                    
                    },
                    {
                        "render": function ( data, type, row ) {
                            return '<a href="{{ url('/patients_ipd/patientBill/print/') }}/'+row[0] +'"target="_blank" ><i class="fa fa-pencil-square-o" aria-hidden="true"></i>Print</a>';
                        },
                        "sortable": false,
                        "targets": 8                   
                    },
                    {
                        "render": function ( data, type, row ) {
                            return '<a href="{{ url('/patients_ipd/patientBill/printReceipt/') }}/'+row[0] +'"target="_blank" ><i class="fa fa-pencil-square-o" aria-hidden="true"></i>Print Bill Receipt</a>';
                        },
                        "sortable": false,
                        "targets": 9                
                    }
        ...
        function doDelete(id) {
            if(confirm('You really want to delete this record?')) {
                ...
                $.ajax({ url: '{{ url('/patients_ipd/patientBill/delete') }}/' + id, 
               type: 'DELETE',
