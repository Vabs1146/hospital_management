...
function calculate_sph() {
	...
	if(!isNaN(parseFloat(sph)) && !isNaN(parseFloat(add))) {
		sph = (!isNaN(parseFloat(sph))) ? parseFloat(sph) : 0;
		add = (!isNaN(parseFloat(add))) ? parseFloat(add) : 0;
	...
	$('#refraction_table input[name="r_nv_sph"]').val('');
	}
}
...
var sph = $('#refraction_table input[name="l_dv_sph"]').val();
...
function calculate_sph_l() {
	...
	$('#refraction_table input[name="l_nv_sph"]').val('');
	}
}
