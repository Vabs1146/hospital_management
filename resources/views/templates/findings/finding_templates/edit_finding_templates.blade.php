...
@endif      
<div class="card">
{{-- Form::model($casedata, array('url' => url('update-findings-template').'/'.$template_id.'/'.$case_id, 'method' => 'POST','id'=>'new',  'class' => 'form-horizontal' )) --}}
...
{{-- Form::model($casedata, array('url' => url('update-prescription-template'), 'method' => 'POST','id'=>'eyeform',  'class' => 'form-horizontal', 'enctype' => 'multipart/form-data' )) --}}
	...
	{{ Form::model($casedata, array('url' => url('update-findings-template'), 'method' => 'POST','id'=>'eyeform',  'class' => 'form-horizontal', 'enctype' => 'multipart/form-data' )) }}
{{ csrf_field() }}
<input type="hidden" name="template_id" id="template_id" class="form-control"  value="{{$template_id}}">
<input type="hidden" name="case_id" id="case_id" class="form-control"  value="{{$case_id}}">
...
<div class="row clearfix">
	...
	<div class="col-md-7">
		<div class="form-group">
			<div class="form-line">
				<input type="text" name="template_name" id="template_name" class="form-control" required value="{{$template->name}}">
			</div>
		</div>
	</div> 
