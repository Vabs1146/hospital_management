...
<div class="panel panel-default">
    ...
    <div class="panel-body">
        ...
        <form action="{{ url('/seo/EditSeo' ) }}" method="POST" class="form-horizontal">
            {{ csrf_field() }}
            ...
            <div class="form-group">
			{{ Form::hidden('seo_id', $seolist->id) }}
                {{ Form::label('Page Url', 'url', array('class' => 'col-sm-3 control-label')) }}
                <div class="col-sm-6">
                {{ Form::text('page_url', Request::old('page_url',$seolist->url), array('class' => 'form-control', 'placeholder'=>'Type Page Url','required', 'readonly', 'disabled')) }}
                </div>
            </div>
            <div class="form-group">
                {{ Form::label('meta_title','Meta Title', array('class' => 'col-sm-3 control-label')) }}
                <div class="col-sm-6">
                    {{ Form::text('meta_title', Request::old('meta_title',$seolist->meta_title), array('class' => 'form-control', 'placeholder'=>'Type Meta Title','required')) }}
                </div>
            </div>
			<div class="form-group">
                {{ Form::label('meta_desc','Meta Description', array('class' => 'col-sm-3 control-label')) }}
                <div class="col-sm-6">
                    {{ Form::text('meta_desc', Request::old('meta_desc',$seolist->meta_desc), array('class' => 'form-control', 'placeholder'=>'Type Meta Description')) }}
                </div>
            </div>
			<div class="form-group">
                {{ Form::label('meta_key','Meta Keywords', array('class' => 'col-sm-3 control-label')) }}
                <div class="col-sm-6">
                    {{ Form::text('meta_key', Request::old('meta_key',$seolist->meta_key), array('class' => 'form-control', 'placeholder'=>'Type Meta Keywords')) }}
                </div>
            </div>
            ...
            <div class="form-group"> 
                ...
                <div class="col-sm-6">
				<?php $act =""; $deact=""; if($seolist->status == '1'){$act="checked";}else{$deact = "checked";} ?>
                <div class="checkbox">
                    <label><input type="radio" name="is_active" id="is_active" value="1" <?php echo $act; ?> required /> Enable </label>
                    <label><input type="radio" name="is_active" id="is_active" value="0" <?php echo $deact; ?> required /> Disable </label>
                </div>
                </div>
            </div>
