    ...
    public function edit(Request $request, $id)
    {
        ...
        $dentist_bill = dentist_bill::where('case_id', $case_master->id)->get();
           ...
           $pay_bill=  DB::table('paymentfor')
            ...
            ->select('paymentfor.id', 'paymentfor.case_id', 'dentist_bill.treatmentDone','paymentfor.date','paymentfor.amountPaid','paymentfor.dentist_bill_id','paymentfor.payment_mode')
            ->get();
        ...
        return view('dentist_bill.add', compact('case_master','dentist_bill','pay_bill','treatment', 'DateWiseRecordLst', 'payment_modes', 'payment_modes_array'));
    }
 ...
 $blnc= ((int)$balance[0]->balance -(int)$request->amountPaid);
    ...
    $updateblnc=DB::update("UPDATE dentist_bill SET balance='$blnc' WHERE id='$treatmentDone'");
        ...
        if (Input::get('Item_Add')) {
            ...
            $form_details->payment_mode=$request->payment_mode;
            $form_details->fill($request->all());
            $form_details->save();
        }
