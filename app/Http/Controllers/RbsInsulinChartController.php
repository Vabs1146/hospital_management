...
class RbsInsulinChartController extends AdminRootController
{
    ...
    public function add(Request $request, $registration_id)
    {
       ...
       $rbs_insulin_chart_record = DB::table('rbs_insulin_chart')->where('registration_id', $registration_id)->first();
       
       //dd($daily_order_sheet_record);
       
       if($rbs_insulin_chart_record) {
            $rbs_insulin_chart_data = DB::table('rbs_insulin_chart_data')->where('rbs_insulin_chart_id', $rbs_insulin_chart_record->id)->get();
       } else {
            $daily_order_sheet_data = [];
       }
       
       //dd($daily_order_sheet_data);
        return view('rbs_insulin_chart.add', compact('registration_data', 'rbs_insulin_chart_record', 'rbs_insulin_chart_data', 'registration_id'));
    }
    ...
    function save(Request $request, $registration_id = "") {
     //echo "============>>>>>> <pre>".__LINE__; print_r($_POST); exit;
        ...
        DB::table('rbs_insulin_chart')->updateOrInsert( $insert_data, $where);
        
        $rbs_insulin_chart_record = DB::table('rbs_insulin_chart')->where('registration_id', $request->registration_id)->first();
        
           
        //================================= start ===================================
        if(!empty($request->rbs_insulin_chart_old_id)) {          
            
            foreach($request->rbs_insulin_chart_old_id as $key => $row) {
                
                
                if($request->rbs_insulin_chart_date[$key] != '') {
            //echo "============".__LINE__; exit;
                    
                    $insert_data = array(
                        'rbs_insulin_chart_id' => $rbs_insulin_chart_record->id,
                        'date' => $request->rbs_insulin_chart_date[$key],
                        'time' => $request->rbs_insulin_chart_time[$key],
                        'hgt' => $request->rbs_insulin_chart_hgt[$key],
                        'u_sugar' => $request->rbs_insulin_chart_u_sugar[$key],
                        'u_acetone' => $request->rbs_insulin_chart_u_acetone[$key],
                        'insuline_given' => $request->rbs_insulin_chart_insuline_given[$key]
                    );
                    
                     
                    if($row) {
                     //echo "<hr>=======>>>>>11111111111>>>>>>> <pre>".__LINE__; print_r($insert_data); exit;
                        DB::table('rbs_insulin_chart_data')->where('id', $row)->update($insert_data);  
                    } else {
                     //echo "<hr>=======>>>>>11111111111>>>>>>> <pre>".__LINE__; print_r($insert_data); exit;
                        DB::table('rbs_insulin_chart_data')->insert($insert_data);    
                    }
                }
            } 
        }
        
        if(isset($request->rbs_insulin_chart_all_old_id)) {
            foreach($request->rbs_insulin_chart_all_old_id as $rbs_insulin_chart_all_old_id_val) {
                if($rbs_insulin_chart_all_old_id_val != "" && !in_array($rbs_insulin_chart_all_old_id_val, $request->rbs_insulin_chart_old_id)) {
                    DB::table('rbs_insulin_chart_data')->where('id', $rbs_insulin_chart_all_old_id_val)->delete();  
                }
            }
        }
    ...
    public function print(Request $request, $registration_id) {
       ...
       //dd($daily_order_sheet_data);
        //return view('rbs_insulin_chart.print', compact('registration_data', 'rbs_insulin_chart_record', 'rbs_insulin_chart_data', 'registration_id'));
        ...
        return view('rbs_insulin_chart.print', compact('registration_data', 'rbs_insulin_chart_record', 'rbs_insulin_chart_data', 'registration_id', 'logoUrl', 'FooterUrl'));
    }
