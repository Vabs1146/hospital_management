...
class TprMonitoringChartController extends AdminRootController
{
    ...
    public function add(Request $request, $registration_id)
    {
       ...
       $main_record = DB::table('tpr_monitoring_chart')->where('registration_id', $registration_id)->get();
      ...
      // dd($main_record);
       ...
       //dd($tpr_monitoring_chart_data);
        return view('trp_monitoring_chart.add', compact('registration_data', 'tpr_monitoring_chart_data', 'registration_id'));
    }
    ...
    //echo "============>>>>>> <pre>".__LINE__; print_r($_POST); exit;
     ...
     foreach($request->date as $key => $date_val) {
         foreach($request->timing[$key] as $timing_key => $timing_val) {
            ...
            // dd($timing_val);
             ...
             $insert_data = [
                ...
                'date'              => $date,
                'timing'            => $request->timing[$key][$timing_key],
                'temp'              => $request->temp[$key][$timing_key],
                'pulse'             => $request->pulse[$key][$timing_key],
                'resp'              => $request->resp[$key][$timing_key],
                'bp'                => $request->bp[$key][$timing_key],
                'spo2'              => $request->spo2[$key][$timing_key],
                'iv'                => $request->iv[$key][$timing_key],
                'rt'                => $request->rt[$key][$timing_key],
                'oral'              => $request->oral[$key][$timing_key],
                'ag'                => $request->ag[$key][$timing_key],
                'urine'             => $request->urine[$key][$timing_key],
                'aspiration'        => $request->aspiration[$key][$timing_key],
                //'created_by'        => Auth::user()->id 
            ];
            
            if(isset($request->record_id[$key][$timing_key]) && $request->record_id[$key][$timing_key] != "") {
                
                 $insert_data['updated_by'] = Auth::user()->id;
                 $insert_data['updated_at'] = date('Y-m-d h:i:s');
                 
                echo "<hr>Update : ". $request->record_id[$key][$timing_key];
                DB::table('tpr_monitoring_chart')->where('id', $request->record_id[$key][$timing_key])->insert($insert_data);
            } else {
                $insert_data['created_by'] = Auth::user()->id;
                echo "<hr>Insert : ". $request->record_id[$key][$timing_key];
                DB::table('tpr_monitoring_chart')->insert($insert_data);
            }
