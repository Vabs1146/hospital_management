...
class Medical_storesotherController extends AdminRootController
{
  ...
  public function __construct()
    ...
    {
        return view('medical_stores_other.index', []);
    }

    public function medicie_list(Request $request)
    {
        return view('medical_stores_other.medicine_index', []);
    }

    public function create(Request $request)
    {
        return view('medical_stores_other.add', [
            []
        ]);
    }
   ...
   $user = Auth::user()->id;
        ...
        if ($this->acc == 1) {
             return view('medical_stores_other.medicine_add', [
            []
        ]);
        }
       else
    ...
    {
        $medical_store = entmedical_store::findOrFail($id);
        return view('medical_stores_other.add', [
            'model' => $medical_store]);
    ...
    public function medicine_edit(Request $request, $id)
    {
        ...
        $this->acc = $this->commonHelper->checkUserAccess("Medicine/edit",Auth::user()->id);
        if ($this->acc == 1) {
    ...
    {
        $medical_store = entmedical_store::findOrFail($id);
        return view('medical_stores_other.show', [
            'model' => $medical_store       ]);
    }
    ...
    public function grid(Request $request)
    {
        ...
        $presql = " FROM entmedical_store a ";
    ...
    public function InsertUpdateMedicine(Request $request)
    {
        ...
        $insertUpdate = "";
        if ($request->id > 0) {
            $medical_store = entmedical_store::findOrFail($request->id);
            $insertUpdate = "Updated";
        } else {
            $medical_store = new entmedical_store;
    ...
    public function update(Request $request)
    {
        ...
        return redirect()->route('medical_stores_other.index')->with('flash_message', 'Record updated successfully');
    }

    public function store(Request $request)
    {
        $this->InsertUpdateMedicine($request);
        return redirect()->route('medical_stores_other.index')->with('flash_message', 'Record inserted successfully');
    }
    ...
    public function medicine_update(Request $request)
    {
        ...
        return redirect("/Other_Medicine")->with('flash_message', 'Record updated successfully');
    }
    ...
    public function medicine_store(Request $request)
    {
        ...
        return redirect("/Other_Medicine")->with('flash_message', 'Record inserted successfully');
    }
