...
class TreatmentMedicationSheetController extends AdminRootController
{
...
$presDropdowns = [
    ...
    //$discharge_summary = $this->convert_to_data_object($discharge_summary_data, []);
        ...
        $treatment_medication_sheet_records = DB::table('treatment_medication_sheet')->where('registration_id', $registration_id)->get();
       
       $final_data = [];
       foreach($treatment_medication_sheet_records as $treatment_medication_sheet_records_row) {
           $final_data[$treatment_medication_sheet_records_row->date]['administration_data'] = DB::table('treatment_medication_sheet_data')->where('medication_id', $treatment_medication_sheet_records_row->id)->where('type', 'administration')->get();
           
           $final_data[$treatment_medication_sheet_records_row->date]['iv_fluid_data'] = DB::table('treatment_medication_sheet_data')->where('medication_id', $treatment_medication_sheet_records_row->id)->where('type', 'iv_fluid')->get();
           $final_data[$treatment_medication_sheet_records_row->date]['non_drug_orders_data'] = DB::table('treatment_medication_sheet_data')->where('medication_id', $treatment_medication_sheet_records_row->id)->where('type', 'non_drug_orders')->get();
           $final_data[$treatment_medication_sheet_records_row->date]['nutrition_data'] = DB::table('treatment_medication_sheet_data')->where('medication_id', $treatment_medication_sheet_records_row->id)->where('type', 'nutrition')->get();
       }
      ...
      //dd($final_data);
        ...
        return View('treatment_medication_sheet.discharge_add', compact(
                //'doctorlist', 
                //'payment_modes_array', 
                'registration_data', 
                'discharge_data', 
    ...
    //echo "============>>>>>> <pre>".__LINE__; print_r($_POST); exit;
     ...
     foreach($request->date as $key => $date_val) {
         DB::table('treatment_medication_sheet')->insert(
             [
                 'date' => $date_val,
                 'registration_id'   => $registration_id,
             ]
        );
         $medication_id = DB::getPdo()->lastInsertId();
         //==========================================================================
         foreach($request->administration_record_1[$key] as $row_key => $row_val) {
             $date = $date_val;
             
             $insert_data = [
                'medication_id'             => $medication_id,
                'type'                      => 'administration',
                'administration_record_1'   => $row_val,
                'administration_record_2'   => $request->administration_record_2[$key][$row_key],
                'administration_record_3'   => $request->administration_record_3[$key][$row_key],
                'administration_record_4'   => $request->administration_record_4[$key][$row_key]
            ];
             
             
            if(isset($request->administration_record_id[$key][$row_key]) && $request->administration_record_id[$key][$row_key] != "") {
               echo "<hr>Update : ". $request->administration_record_id[$key][$row_key];
               DB::table('treatment_medication_sheet_data')->where('id', $request->administration_record_id[$key][$row_key])->update($insert_data);
            } else {
               //$insert_data['created_by'] = Auth::user()->id;
               echo "<hr>Insert : ". $request->administration_record_id[$key][$row_key];
               DB::table('treatment_medication_sheet_data')->insert($insert_data);
            }
         ...
         //===============================iv_fluid_1===========================================
         foreach($request->iv_fluid_1[$key] as $row_key => $row_val) {
             $date = $date_val;
             ...
             $insert_data = [
                ...
                'type'                      => 'iv_fluid',
                'administration_record_1'   => $row_val,
                'administration_record_2'   => $request->iv_fluid_2[$key][$row_key],
                'administration_record_3'   => $request->iv_fluid_3[$key][$row_key],
                'administration_record_4'   => $request->iv_fluid_4[$key][$row_key],
                'administration_record_5'   => $request->iv_fluid_5[$key][$row_key]
            ];
             
             
            if(isset($request->iv_fluid_id[$key][$row_key]) && $request->iv_fluid_id[$key][$row_key] != "") {
               echo "<hr>Update : ". $request->iv_fluid_id[$key][$row_key];
               DB::table('treatment_medication_sheet_data')->where('id', $request->iv_fluid_id[$key][$row_key])->insert($insert_data);
            } else {
               //$insert_data['created_by'] = Auth::user()->id;
               echo "<hr>Insert : ". $request->iv_fluid_id[$key][$row_key];
               DB::table('treatment_medication_sheet_data')->insert($insert_data);
            }
            $final_data[$date][] = $insert_data;
         }
         ...
         //================================non_drug_orders==========================================
         foreach($request->non_drug_orders[$key] as $row_key => $row_val) {
             $date = $date_val;
             
             $insert_data = [
                'medication_id'             => $medication_id,
                'type'                      => 'non_drug_orders',
                'single_val'   => $row_val
            ];
             
             
            if(isset($request->non_drug_orders_id[$key][$row_key]) && $request->non_drug_orders_id[$key][$row_key] != "") {
               echo "<hr>Update : ". $request->non_drug_orders_id[$key][$row_key];
               DB::table('treatment_medication_sheet_data')->where('id', $request->non_drug_orders_id[$key][$row_key])->insert($insert_data);
            } else {
               //$insert_data['created_by'] = Auth::user()->id;
               echo "<hr>Insert : ". $request->non_drug_orders_id[$key][$row_key];
               DB::table('treatment_medication_sheet_data')->insert($insert_data);
            }
            $final_data[$date][] = $insert_data;
         }
         ...
         //============================nutrition==============================================
         foreach($request->nutrition[$key] as $row_key => $row_val) {
             $date = $date_val;
            ...
            if(isset($request->nutrition_id[$key][$row_key]) && $request->nutrition_id[$key][$row_key] != "") {
               echo "<hr>Update : ". $request->nutrition_id[$key][$row_key];
               DB::table('treatment_medication_sheet_data')->where('id', $request->nutrition_id[$key][$row_key])->insert($insert_data);
            } else {
               //$insert_data['created_by'] = Auth::user()->id;
               echo "<hr>Insert : ". $request->nutrition_id[$key][$row_key];
               DB::table('treatment_medication_sheet_data')->insert($insert_data);
            }
            $final_data[$date][] = $insert_data;
         }
